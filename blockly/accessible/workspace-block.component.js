...
blocklyApp.WorkspaceBlockComponent = ng.core.Component({
  ...
  template: `
    <li [id]="componentIds.blockRoot" role="treeitem"
        [attr.aria-labelledBy]="generateAriaLabelledByAttr(componentIds.blockSummary, 'blockly-translate-workspace-block', componentIds)"
...
.Class({
  ...
  addInteriorLink: function(connection) {
    ...
    setTimeout(function(){
      let blocklyAttachToLinkBtn = document.getElementById('blocklyAttachToLinkBtn');
      blocklyAttachToLinkBtn.click();
    }, 500);
  },
  getBlockDescription: function() {
    ...
    if (parentBlock) {
      var fullDescription = blockDescription;
      // + ' inside ' + this.utilsService.getBlockDescription(parentBlock);
  ...
  getBlockNeededLabel: function(blockInput) {
    ...
    return 'Se necesita un ' + blockTypeLabel + ' '  + inputTypeLabel + ': ' ;
  },
  generateAriaLabelledByAttr: function(mainLabel, secondLabel, componentIds) {
