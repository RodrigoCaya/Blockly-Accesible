...
blocklyApp.FieldSegmentComponent = ng.core.Component({
  ...
  template: `
    ...
    <template [ngIf]="mainField">
      <template [ngIf]="isTextInput()">
        {{getPrefixText()}}
        <label [id]="mainFieldId" aria-hidden="true" hidden>
          {{getFieldDescription() + '. ' + ('PRESS_ENTER_TO_EDIT_TEXT'|translate)}}
        </label>
      ...
      <template [ngIf]="isNumberInput()">
        {{getPrefixText()}}
        <label [id]="mainFieldId" aria-hidden="true" hidden>
          {{getFieldDescription() + '. ' + ('PRESS_ENTER_TO_EDIT_NUMBER'|translate)}}
        </label>
  ...
  `,
  inputs: ['prefixFields', 'mainField', 'mainFieldId', 'level', 'tree'],
...
.Class({
  ...
  setDropdownValue: function(optionValue) {
    ...
    if (!optionText) {
      throw Error(
          'No hay opción de texto correspondiente al valor: ' +
          this.optionValue);
    }
