  ...
  template: `
    <div class="blocklySidebarColumn">
      ...
      <button id="{{ID_FOR_ATTACH_TO_LINK_BUTTON}}"
              ...
              class="blocklySidebarButton" hidden>
        {{'ATTACH_NEW_BLOCK_TO_LINK'|translate}}
      </button>
      <button id="{{ID_FOR_CREATE_NEW_GROUP_BUTTON}}"
              (click)="showToolboxModalForCreateNewGroup()"
              class="blocklySidebarButton">
        {{hasVariableCategory() ? 'Crear nuevo grupo de bloques' : (isWorkspaceEmpty() ? 'Último botón: Crear nuevo grupo de bloques' : 'Crear nuevo grupo de bloques')}}
      </button>
      <button id="clear-workspace" (click)="clearWorkspace()"
              ...
              class="blocklySidebarButton">
        {{hasVariableCategory() ? 'Borrar espacio de trabajo' : (isWorkspaceEmpty() ? 'Borrar espacio de trabajo' : 'Último botón: Borrar espacio de trabajo')}}
      </button>
      <button *ngIf="hasVariableCategory()" id="add-variable"
              (click)="showAddVariableModal()"
              class="blocklySidebarButton">
        Último botón: Agregar variable
      </button>
    </div>
  `,
